    ...
    public void startController(){
        ...
        switch (sc.nextLine()){
            case "1":{
                ...
                if(employeeInfo.getWorkState() == 0){
                    employeeInfo = null;
                }
    ...
    public boolean loginController() throws Exception{
        ...
        switch (sc.nextLine()){
            ...
            case "b":{
                ...
                ServerStart.showAllJob();
                int jobId = Integer.parseInt(sc.nextLine());
                int departId = employeeInfo.getDepart().getDepartId();
                String supName = employeeInfo.getEmpName();
                if(ServerStart.addEmployee(supName,empName,phone,address,addressDetail,card,jobId,departId)){
                    MainView.log("添加成功,将员工加入你的部门和门下");
                }else {
                    MainView.log("添加失败");
                }
                break;
            ...
            case "f":{
                MainView.log("输入惩罚的员工姓名");
                String name = sc.nextLine();
                int empId = ServerStart.getEmployeeIdByName(name);
                if (empId == -1){
                    MainView.log("不存在该员工");
                    return true;
                }
                MainView.log("输入惩罚编号");
                ServerStart.showAllPunish();
                int punishId = Integer.parseInt(sc.nextLine());
                ServerStart.addPunishToEmployee(punishId,empId);
                break;
            }
            case "g":{
                MainView.log("输出奖励员工的姓名");
                String name = sc.nextLine();
                int empId = ServerStart.getEmployeeIdByName(name);
                if(empId == -1){
                    MainView.log("不存在该员工");
                ...
                MainView.log("输入奖励编号");
                ServerStart.showAllBonus();
                int bonusId = Integer.parseInt(sc.nextLine());
                ServerStart.addBonusToEmployee(bonusId,empId);
                break;
            }
            default:{
                break;
